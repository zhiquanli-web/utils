export default class Swiper {
  ...
  wrapContainer() {
    ...
    this.elements.items = this.elements.swiper.querySelectorAll(
      this.options.item
    );
  }
   ...
   */
  initStyle() {
    this.elements.swiper.setAttribute(
      "style",
      `
          position: relative; 
          overflow: hidden;
  ...
  touchStart(event) {
    ...
    this.states.touch = 1;
    this.states.autoplay = false;
    this.states.touchTrack.start = this.states.touchTrack.old = event.touches
      ? event.touches[0]
      : event;
  }
  ...
  change(index) {
    ...
    this.elements.container.addEventListener("transitionend", () => {
      this.elements.container.style.cssText =
        this.elements.container.style.cssText.replace("transition", "");
    });
